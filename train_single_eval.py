...
def training(dataset, opt, pipe, checkpoint_iterations, checkpoint,output_milestone = False, writer = None ):
    first_iter = 0
            ...
            # 
                    ...
                    if iteration >= 10000 and iteration % 100 == 0:
                        with torch.no_grad():
                            ...
                            # 计算LPIPS
                            lpips_loss = loss_fn_alex(image.unsqueeze(0), gt_image.unsqueeze(0)).item()
                            
                            # 获取当前损失值
                            total_loss = loss.item()
                            l1_loss_value = Ll1.item()

                            # 写入CSV文件
                            metrics_path = os.path.join(args.model_path, 'training_metrics.csv')
...
def prepare_output_and_logger(args):    
    ...
    if not os.path.exists(metrics_path):
        with open(metrics_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['iteration', 'psnr', 'ssim', 'lpips', 'loss', 'l1_loss'])
...
if __name__ == "__main__":
    ...
    # Start training
    training(lp.extract(args), op.extract(args), pp.extract(args), args.checkpoint_iterations, args.start_checkpoint, args.output_milestone, writer)
    writer.close()
