...
RasterizeGaussiansCUDA(
	...
	const bool require_max_indices,
	const bool debug)
{
  ...
  torch::Tensor out_depth = torch::full({0, H, W}, 0.0, float_opts);
  torch::Tensor out_mdepth = torch::full({0, H, W}, 0.0, float_opts);
  torch::Tensor out_coord = torch::full({0, H, W}, 0.0, float_opts);
  torch::Tensor out_mcoord = torch::full({0, H, W}, 0.0, float_opts);
  torch::Tensor out_normal = torch::full({0, H, W}, 0.0, float_opts);

	if (require_coord && require_depth)
	{
		// RENDER_CUDA_CALL(true, true, true);
		out_depth = torch::full({1, H, W}, 0.0, float_opts);
		out_mdepth = torch::full({1, H, W}, 0.0, float_opts);    

		out_coord = torch::full({3, H, W}, 0.0, float_opts);
		out_mcoord = torch::full({3, H, W}, 0.0, float_opts);
		out_normal = torch::full({3, H, W}, 0.0, float_opts);
	}
  ...
  torch::Tensor maxweight_indices = torch::full({1, H, W}, 0, int_opts);
  int* maxweight_indices_ptr = nullptr;
  if (require_max_indices)
  {
	maxweight_indices = torch::full({1, H, W}, 0, int_opts);
	maxweight_indices = maxweight_indices.contiguous();
	maxweight_indices_ptr = maxweight_indices.data<int>();
  }
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		maxweight_indices_ptr,
		radii.contiguous().data<int>(),
		require_coord,
		require_depth,
		debug);
  }
  return std::make_tuple(rendered, out_color, out_coord, out_mcoord, out_alpha, out_normal, out_depth, out_mdepth, maxweight_indices, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  float* dL_dts_ptr = nullptr;
  torch::Tensor dL_dcamera_planes = torch::zeros({P, 0}, means3D.options());
  float* dL_dcamera_planes_ptr = nullptr;
  torch::Tensor dL_dray_planes = torch::zeros({P, 0}, means3D.options());
  float* dL_dray_planes_ptr = nullptr;
  torch::Tensor dL_dnormals = torch::zeros({0, 0}, means3D.options());
  float* dL_dnormals_ptr = nullptr;

//   printf("RasterizeGaussiansBackwardCUDA\n");
//   printf("dL_dout_coord size: %d %d %d\n", dL_dout_coord.size(0), dL_dout_coord.size(1), dL_dout_coord.size(2));
//   printf("dL_dout_color size: %d %d %d\n", dL_dout_color.size(0), dL_dout_color.size(1), dL_dout_color.size(2));
//   printf("dL_dout_normal size: %d %d %d\n", dL_dout_normal.size(0), dL_dout_normal.size(1), dL_dout_normal.size(2));
//   printf("dL_dout_depth size: %d %d %d\n", dL_dout_depth.size(0), dL_dout_depth.size(1), dL_dout_depth.size(2));
  if (dL_dout_normal.size(0) != 0)
  {
	dL_dnormals = torch::zeros({P, 3}, means3D.options());
	dL_dnormals = dL_dnormals.contiguous();
	dL_dnormals_ptr = dL_dnormals.data_ptr<float>();
  }
  if (dL_dout_depth.size(0) != 0)
  {
	dL_dts = torch::zeros({P, 1}, means3D.options());
	dL_dts = dL_dts.contiguous();
	dL_dts_ptr = dL_dts.data_ptr<float>();

	dL_dray_planes = torch::zeros({P, 2}, means3D.options());
	dL_dray_planes = dL_dray_planes.contiguous();
	dL_dray_planes_ptr = dL_dray_planes.data_ptr<float>();
  }
  if (dL_dout_coord.size(0) != 0)
  {
	dL_dcamera_planes = torch::zeros({P, 6}, means3D.options());
	dL_dcamera_planes = dL_dcamera_planes.contiguous();
	dL_dcamera_planes_ptr = dL_dcamera_planes.data_ptr<float>();
  }
