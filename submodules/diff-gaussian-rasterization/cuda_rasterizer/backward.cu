	...
	float* dL_dopacity)
{
	...
	const glm::vec3 dL_dnormal = (dL_dnormals == nullptr)
     ? glm::vec3(0.0f, 0.0f, 0.0f)
     : glm::vec3(dL_dnormals[idx*3], 
                 dL_dnormals[idx*3+1],
                 dL_dnormals[idx*3+2]);
	const float4 conic = conic_opacity[idx];
	const float combined_opacity = conic.w;
	const float2 dL_camera_plane0 = 
    (dL_dcamera_planes == nullptr) 
    ? make_float2(0.f, 0.f)
    : dL_dcamera_planes[idx*3];

	const float2 dL_camera_plane1 =
		(dL_dcamera_planes == nullptr)
		? make_float2(0.f, 0.f)
		: dL_dcamera_planes[idx*3 + 1];

	const float2 dL_camera_plane2 =
		(dL_dcamera_planes == nullptr)
		? make_float2(0.f, 0.f)
		: dL_dcamera_planes[idx*3 + 2];

	const float2 dL_dray_plane =
		(dL_dray_planes == nullptr)
		? make_float2(0.f, 0.f)
		: dL_dray_planes[idx];
	...
	glm::vec4* dL_drot)
{
	...
	const float dL_dt = (dL_dts == nullptr) 
    ? 0.0f 
    : dL_dts[idx];
