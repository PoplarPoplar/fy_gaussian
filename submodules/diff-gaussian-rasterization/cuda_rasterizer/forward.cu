	...
	)
{
	...
	if (inside)
	{
		...
		if (maxweight_indices != nullptr)
		{
			maxweight_indices[pix_id] = maxweight_id;
		}
		if constexpr (COORD)
		{
			if(last_contributor)
...
renderCUDA<NUM_CHANNELS, template_coord, template_depth, template_normal> <<<grid, block>>> ( \
	...
	out_coord, out_mcoord, out_normal, out_depth, out_mdepth, maxweight_indices,\
