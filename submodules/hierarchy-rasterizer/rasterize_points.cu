	...
	const bool do_depth)
{
  ...
  torch::Tensor out_maxweight_indices = torch::full({1, H, W}, 0, int_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_maxweight_indices.contiguous().data<int>(), // 可更改为利用ptr
		out_invdepthptr,
  ...
  return std::make_tuple(rendered, out_color, radii, pixels, geomBuffer, binningBuffer, imgBuffer, out_invdepth, out_maxweight_indices);
}
