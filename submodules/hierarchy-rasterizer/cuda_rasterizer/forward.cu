...
renderCUDA(
	...
	int* __restrict__ out_maxweight_indices,
	int P, int skyboxnum,
	...
	float* __restrict__ invdepth)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (alpha * T > maxweight)
			{
				maxweight = alpha * T;
				maxweight_id = coll_id;
			}
	...
	if (inside)
	{
		...
		out_maxweight_indices[pix_id] = maxweight_id;
