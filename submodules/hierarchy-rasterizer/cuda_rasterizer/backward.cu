...
// each Gaussian.
__device__ void computeColorFromSH(int idx, int t_idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	...
	dL_dRGB.x *= clamped[3 * t_idx + 0] ? 0 : 1;
	dL_dRGB.y *= clamped[3 * t_idx + 1] ? 0 : 1;
	dL_dRGB.z *= clamped[3 * t_idx + 2] ? 0 : 1;
	...
	float* dL_dopacity)
{
	...
	if (shs)
		computeColorFromSH(idx, t_idx, D, M, (glm::vec3*)means, *campos, shs, clamped, (glm::vec3*)dL_dcolor, (glm::vec3*)dL_dmeans, (glm::vec3*)dL_dsh);
	...
	{
		int parent_id = parent_indices[t_idx];
		if(parent_id == -1)
			return;

		float t = ts[t_idx];

		float dl_opacity = dL_dopacity[idx];
		dL_dopacity[idx] *= 0;

		glm::vec3 dl_scale = dL_dscale[idx];
		dL_dscale[idx] *= 0;

		glm::vec4 dl_rot = dL_drot[idx];
		dL_drot[idx] *= 0;

		float rotmult = 1.0f;
		glm::vec4 rot = rotations[idx];
		glm::vec4 otherrot = rotations[parent_id];
		...
		glm::vec3 dl_mean = dL_dmeans[idx];
		dL_dmeans[idx] *= 0;
		for(int i = 0; i < 3; i++)
		 	atomicAdd(((float*)(dL_dmeans + parent_id)) + i, (1.0f - t) * dl_mean[i]);

		for(int i = 0; i < 48; i++)
		{
			float dl_sh = dL_dsh[idx * 48 + i];
			dL_dsh[idx * 48 + i] *= 0;
		}
	}
}
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float test_alpha = con_o.w * G;
			bool nullalpha = test_alpha > 0.99f;

			int global_id = collected_id[j];

			float my_alpha = min(0.99f, test_alpha);
			float alpha;
			float t, frac;
			if (ts != nullptr && kids != nullptr)
			{
				...
				float kidsqrt_alpha = 1.0f - pow(1.0f - my_alpha, frac);
			...
			atomicAdd(&(dL_dinvdepths[global_id]), weight * dL_invdepth);
			}
			...
			dL_dalpha = nullalpha ? 0 : dL_dalpha;
			...
			float opac_grad_mult = 1.0f;
			if(ts != nullptr && kids != nullptr)
			{
				opac_grad_mult = t - pow(1.0f - my_alpha, frac - 1.0f) * (t - 1.0f) * frac;
			}

			// Update gradients w.r.t. opacity of the Gaussian
			atomicAdd(&(dL_dopacity[global_id]), opac_grad_mult * G * dL_dalpha);
		}
	}
}
