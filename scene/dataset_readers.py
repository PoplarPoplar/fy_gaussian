...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, masks_folder, depths_folder, normals_folder, test_cam_names_list):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        image_path = os.path.join(images_folder, f"{extr.name}") if images_folder != "" else ""
        image_name = extr.name
        mask_path = os.path.join(masks_folder, f"{extr.name[:-n_remove]}.png") if masks_folder != "" else ""
        depth_path = os.path.join(depths_folder, f"{extr.name[:-n_remove]}.png") if depths_folder != "" else ""
        normal_path = os.path.join(normals_folder, f"{extr.name}") if normals_folder != "" else ""

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, primx=primx, primy=primy, depth_params=depth_params,
                              image_path=image_path, mask_path=mask_path, depth_path=depth_path, normal_path=normal_path, image_name=image_name, 
...
def readColmapSceneInfo(path, images, masks, depths, normals, eval, train_test_exp, llffhold=None):
    try:
    ...
    images = "" if images == None else images
    masks_reading_dir = masks if masks == "" else os.path.join(path, masks)

    cam_infos_unsorted = readColmapCameras(
        cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, depths_params=depths_params, 
        images_folder=os.path.join(path, images) if images!= "" else "", 
