...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], create_from_hier=False):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.alpha_masks, args.depths, args.normals, args.eval, args.train_test_exp)
        else:
            assert False, "Could not recognize scene type!"

        if not self.loaded_iter and self.loaded_iter != "make_chunk":
        ...
        if len(args.init_gs_path) > 0:
            self.gaussians.create_from_init_gs(args.init_gs_path, 
                                               args.bounds_file,
                                               ...
                                               args.skybox_locked,
                                               args.use_sphere)
        elif self.loaded_iter and self.loaded_iter != "make_chunk":
